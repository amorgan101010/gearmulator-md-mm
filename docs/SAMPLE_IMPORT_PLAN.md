...
Implemented on branch `md-mm-remote-panel`:

- **2.1 done.** `mdLibTest/mdSampleRateProbe.cpp`: 1 kHz tones sent with 44.1 kHz and 32 kHz SDS headers both
  play back at 1000.0 Hz. The machine honours the header rate, so imports keep the file's own rate. Only
  files above 48 kHz are resampled (to 44.1 kHz, two biquad low-passes at 0.45 x 44.1 kHz + Lagrange).
- **2.2 done.** `mdLib/mdsdsencode.h/.cpp` + `mdLibTest/sdsEncodeTest.cpp` (validator acceptance, bit-exact round trip).
- **2.3 done.** `mdJucePlugin/mdSampleImport.h/.cpp`: decode via `AudioFormatManager::registerBasicFormats`
  (this build has JUCE's FLAC/MP3/Ogg codecs disabled; WAV and AIFF always work, MP3/M4A/CAF via CoreAudio on
  macOS), average to mono, 16 bit, name = first 4 characters of the file stem.
- **2.4 done.** Right-click menu "Load Samples (WAV, AIFF)..." (Machinedrum only) -> multi-select chooser ->
  slot menu (R01..R48, names from the ledger, first free run pre-checked) -> memory check -> existing SDS transfer
  with its ready-to-receive confirmation and progress/cancel UI. `Editor::sendUserSysexFile` was split so
  `sendUserSysexBytes` can take generated bytes.
- **2.5 done (ledger variant).** Config key `mdSampleSlotLedger` ("slot:words:name;..."), updated when a sample
  transfer completes; the memory check counts ledger entries outside the target slots. Not stored in the project.
- **2.6 done.** `mdJucePlugin/mdSampleDropTarget.h/.cpp` on the document body. Needed a framework opt-in:
  `juceRmlUi::DragTarget::setAcceptDragsOverChildren(true)` (the base class only accepted drags whose innermost
  element is the target itself). One `.syx` file (both machines) or any number of audio files (Machinedrum).
- Test: `mdJucePlugin/mdSampleImportTest.cpp` (ctest `mdSampleImportTest`): writes WAVs with JUCE, checks decode,
  mono mix, 96 kHz resampling, multi-slot stream validity, ledger; with `GEARMULATOR_MD_FIRMWARE_BIN` and
  `GEARMULATOR_MD_FACTORY_CACHE` it transfers into an emulated MD and checks each slot's playback pitch.

Open:
- Drop onto a trig key to target that track's slot and assign the ROM machine (not done).
- Reading slot names/usage back from the machine (X.13 status 0x34) instead of the plugin-side ledger.
- Manual verification in Bitwig (menu flow, Finder drop, Bitwig clip drop).
- Remote panel upload (2.7).

Pitfall found on the way: do not add data members to `dsp56k::Jit`; doing so kept the MD firmware from enabling
MIDI receive (see memory note).
